...
SOURCE_DIR = ROOT / "data/external/mv_physical_validation"
DEFAULT_SOURCE = SOURCE_DIR / "lyons_lineage_provenance_v2.json"
DEFAULT_OUTPUT = ROOT / "outputs/validation/mv_physical"
...
def build_report(source: dict) -> dict:
    ...
    return {
        ...
        "project_status_basis": {
            ...
            "path": "outputs/validation/mv_physical/mv_physical_validation_v1.json",
        ...
        "generated_data_files": ["lyons_front_digitized_manuscript_v2.csv"],
...
def render_markdown(source: dict, report: dict) -> str:
    ...
    lines = ["# Lyons constitutive precheck v2", "", f"Status: **{INSUFFICIENT}**", "",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        (args.output_dir / "lyons_constitutive_precheck_v2.json").write_text(json.dumps(report, indent=2, allow_nan=False)+"\n", encoding="utf-8")
        (args.output_dir / "lyons_constitutive_precheck_v2.md").write_text(f"# Lyons v2\n\n{INSUFFICIENT}\n\nProject status preserved: {PROJECT_STATUS}\n\nInvalid source: {error}\n\nNo data files generated by this run.\n", encoding="utf-8")
        print(f"{INSUFFICIENT}: {error}")
        return 2
    args.output_dir.mkdir(parents=True, exist_ok=True)
    (args.output_dir / "lyons_constitutive_precheck_v2.json").write_text(encoded, encoding="utf-8")
    (args.output_dir / "lyons_constitutive_precheck_v2.md").write_text(render_markdown(source, report), encoding="utf-8")
    with (args.output_dir / "lyons_front_digitized_manuscript_v2.csv").open("w", encoding="utf-8", newline="") as stream:
