...
DEFAULT_PROVENANCE = (
    ROOT / "data" / "external" / "mv_physical_validation" / "source_provenance_v1.json"
)
DEFAULT_OUTPUT_DIR = ROOT / "outputs" / "validation" / "mv_physical"
...
def write_outputs(report: Mapping[str, object], output_dir: Path) -> None:
    ...
    phases = {
        "phase_a_transport_closure_v1": report["phase_a"],
        "phase_b_magnetic_drift_validation_v1": report["phase_b"],
        "phase_c_dipolar_w_validation_v1": report["phase_c"],
        "phase_d_joint_mv_validation_v1": report["phase_d"],
    }
    ...
    _write_json(output_dir / "mv_physical_validation_v1.json", report)
    _write_markdown(output_dir / "mv_physical_validation_v1.md", _report_markdown(report))
...
def _phase_markdown(phase: Mapping[str, object]) -> str:
    lines = [
        f"# MV physical validation — Phase {phase['phase']}",
        "",
...
def _report_markdown(report: Mapping[str, object]) -> str:
    ...
    return f"""# MV physical validation v1
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        _write_json(args.output_dir / "mv_physical_validation_v1.json", blocked)
        _write_markdown(
            args.output_dir / "mv_physical_validation_v1.md",
