...
SOURCE_PROVENANCE_RELATIVE_PATH = Path("data/external/mv_physical_validation/source_provenance_v1.json")
...
def default_source_provenance_path() -> Path:
    ...
    return Path(__file__).resolve().parents[3] / SOURCE_PROVENANCE_RELATIVE_PATH
...
def assess_magnetic_2d_closure(
    ...
    if not path.is_file():
        return _blocked(path, ("source_provenance_v1.json is absent",))
    try:
